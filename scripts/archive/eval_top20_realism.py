...
def analyze_top20(df_train: pd.DataFrame, df_test: pd.DataFrame, label: str):
    ...
    # Filter for Top 20 vs Top 20
    sub_df = df_test[(df_test["h_rank"] <= 20) & (df_test["a_rank"] <= 20)].copy()
    if len(sub_df) == 0:
        return {}
    ...
    # Analyze upsets and probabilities
    is_home_fav = sub_df["elo_diff"] > 0
    actual_fav_won = np.where(is_home_fav, y_te == 0, y_te == 2)
    actual_draw = (y_te == 1)
    actual_upset = np.where(is_home_fav, y_te == 2, y_te == 0) # Favorite loses
    
    prob_fav_wins = np.where(is_home_fav, proba[:, 0], proba[:, 2])
    prob_draw = proba[:, 1]
    prob_upset = np.where(is_home_fav, proba[:, 2], proba[:, 0])
    
    return {
        "n": len(sub_df),
        "ll": log_loss(y_te, proba, labels=[0,1,2]),
        "ece": calculate_ece(y_te.values, proba),
        "avg_prob_fav": prob_fav_wins.mean(),
        "avg_prob_draw": prob_draw.mean(),
        "avg_prob_upset": prob_upset.mean(),
        "actual_fav_rate": actual_fav_won.mean(),
        "actual_draw_rate": actual_draw.mean(),
        "actual_upset_rate": actual_upset.mean(),
    }
...
def main():
    ...
    b_res = analyze_top20(train_base, test_identical, "Baseline")
    s_res = analyze_top20(train_s1, test_identical, "Scenario 1")

    print("\n=== KNOCKOUT-STAGE REALISM AUDIT (Top-20 vs Top-20, 2023+ Matches) ===")
    print(f"Number of Matches: {b_res['n']}")
    
    print("\n1. Calibration & Performance")
    print(f"   Metric        | Baseline | Scenario 1")
    print(f"   -------------------------------------")
    print(f"   Log Loss      | {b_res['ll']:.4f}   | {s_res['ll']:.4f}")
    print(f"   ECE           | {b_res['ece']:.4f}   | {s_res['ece']:.4f}")
    
    print("\n2. Predicted Probabilities vs Actual (Elo Favorite Perspective)")
    print(f"   Outcome    | Actual Freq | Base Pred | S1 Pred")
    print(f"   ----------------------------------------------")
    print(f"   Fav Wins   | {b_res['actual_fav_rate']:.4f}      | {b_res['avg_prob_fav']:.4f}    | {s_res['avg_prob_fav']:.4f}")
    print(f"   Draw       | {b_res['actual_draw_rate']:.4f}      | {b_res['avg_prob_draw']:.4f}    | {s_res['avg_prob_draw']:.4f}")
    print(f"   Upset      | {b_res['actual_upset_rate']:.4f}      | {b_res['avg_prob_upset']:.4f}    | {s_res['avg_prob_upset']:.4f}")

    print("\nAnalysis:")
    print("If 'S1 Pred Fav Wins' > 'Base Pred Fav Wins', Scenario 1 leans more heavily on Elo.")
    print("If 'S1 Pred Draw' < 'Actual Draw', it underestimates the likelihood of extra time.")
